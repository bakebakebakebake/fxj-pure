...
description: 顺着一次真实请求，看 DNS、TCP、TLS、HTTP 和拥塞控制怎样分工协作。
...
HTTP、HTTPS、TCP、UDP、DNS、IP、MAC、ARP、拥塞控制、流量控制——这些协议单独看都较为重要。但它们的本职工作是接力完成一次真实通信。
...
> DNS 把”人类友好的名字”和”机器可路由的地址”解耦了，同时还能借此做缓存和流量调度。
...
TCP 是全双工的：一端不再发送，不代表另一端也立刻没数据可发。所以关闭两端发送方向需要分开确认，这就是四次挥手比三次握手多一步的原因。关闭连接还涉及半关闭和 `TIME_WAIT` 等状态。
...
“TCP 可靠，UDP 不可靠”概括了核心区别，但 TCP 的可靠是靠几组机制共同实现的：
...
上层应用通常不想自己重新实现一遍可靠传输。HTTP、MySQL 协议、绝大多数面向正确性交互的后端服务，都直接站在 TCP 的可靠字节流上构建，这也是 TCP 在传统互联网应用里长期作为主力传输协议的原因。
...
HTTPS 比 HTTP 慢，不只是因为加密计算，还因为多了几轮往返：

- 前面先有 TCP 握手；
- 上面要走 TLS 协商；
- 证书校验、密钥交换、握手往返都会带来额外成本。

换来的，是明文 HTTP 不具备的安全边界。
...
HTTP 不负责重传、丢包恢复、流量控制，这些是 TCP 或 QUIC 那一层在处理。HTTP 负责的是请求与响应的表达：
...
每一代 HTTP 都在减少额外开销、提高并发传输效率、降低阻塞带来的连锁影响。
...
TCP 提供了可靠传输，但并非所有场景都需要。UDP 的特点：
...
所以像音视频、实时交互、某些游戏场景，会更愿意站在 UDP 或基于 UDP 的协议之上重新设计传输策略。QUIC 则把很多传统上依赖 TCP 的传输能力，搬到了用户态协议层重新组织。

> [!note]- TCP 与 UDP 的取舍
> TCP 把顺序、确认、重传、窗口、拥塞控制这套机制内建了，适合通用可靠传输；UDP 保持轻量，把更多选择权留给应用层，更适合实时性优先或自定义传输语义的场景。
...
1. 解析 URL，确认协议、域名、端口和资源路径。
2. DNS 把域名翻译成 IP。
3. TCP 建连，如果是 HTTPS 再补 TLS 握手。
4. HTTP 请求与响应，以及各版本差异。
5. 传输层的可靠性、拥塞控制，以及浏览器继续拉静态资源和渲染页面。
...
- TLS 负责让通道可信且保密；
- HTTP 负责表达请求与响应的语义；
- 流量控制、拥塞控制、重传等机制负责传输的稳定和效率。

这整套协议在为同一件事接力：**把一次通信尽量正确、高效、安全地送到对面去。**